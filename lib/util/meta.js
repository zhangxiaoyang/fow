...
Meta.prototype.loadMeta = function() {
  var metaString = fs.readFileSync(this.metaPath, 'utf8');
  var newMeta = [];
  metaString.split('\n').forEach(function(line) {
    if(line.search(/s?create_time/) >= 0
      || line.search(/s?update_time/) >= 0) {
      line = line.split(':')[0] + ": !!str '" + line.split(':').slice(1).join(':').trim() + "'";
    }
    newMeta.push(line);
  });

  this.meta = yaml.safeLoad(newMeta.join('\n'));
};
...
Meta.prototype.updateMeta = function() {
  ...
  fs.writeFile(this.lastBuildJson, JSON.stringify(function(){var json = {}; json[workspaceDir] = new Date(); return json;}()), function(){});
