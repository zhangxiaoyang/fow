...
var getBookFilePath = function(site, book) {
  return path.join(site, book + '.html');
};
...
var SiteHandler = module.exports = function(cwd, option, fow) {
  ...
  var render = new Render(
    path.join(fow.workspace, 'templates'),
    yaml.safeLoad(fs.readFileSync(path.join(fow.workspace, fow.config), fow.encoding)),
    fow.encoding
  );
  ...
  for(var book in update_books) {
    ...
    fse.outputFileSync(
      ...
      render.render(path.join(update_books[book], 'book.yml'), Render.BOOK),
      fow.encoding
    );
  }
